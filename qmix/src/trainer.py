...
class QMIXTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        print(f"   Global state dim: {self.env_info['global_state_dim']}")
        ...
        # 对于可能动态变化状态维度的环境（如Multiwalker），使用最大维度
        env_name = config['env']['name']
        if env_name == 'multiwalker':
            # Multiwalker最大状态维度：3个智能体 * 31维 = 93（基于实际观测）
            max_state_dim = 93
            if self.env_info['global_state_dim'] != max_state_dim:
                print(f"🔧 更新全局状态维度: {self.env_info['global_state_dim']} -> {max_state_dim}")
                self.env_info['global_state_dim'] = max_state_dim
        else:
            # 其他环境使用实际检测到的维度
            test_obs, _ = self.env.reset()
            test_global_state = self.env.get_global_state()
            actual_state_dim = test_global_state.shape[0]

            # 如果实际状态维度与报告的不匹配，更新环境信息
            if actual_state_dim != self.env_info['global_state_dim']:
                print(f"🔧 更新全局状态维度: {self.env_info['global_state_dim']} -> {actual_state_dim}")
                self.env_info['global_state_dim'] = actual_state_dim

        # 创建网络
        self.networks = QMIXNetworks(self.env_info, config, self.device)

        # 创建算法
        self.algorithm = QMIX(self.networks, config, self.device)
        ...
        for episode in range(self.total_episodes):
            ...
            if episode % 100 == 0:
                ...
                print(f"Episode {episode:6d} | Avg Reward: {avg_reward:6.2f} | "
                      f"Avg Length: {avg_length:4.1f} | Epsilon: {self.epsilon:.3f} | "
    ...
    def collect_episode(self) -> Tuple[float, int]:
        ...
        while True:
            ...
            # 选择动作
            actions = self.algorithm.select_actions(obs_tensor, self.epsilon, avail_actions)
            
            # 环境交互
            next_obs, rewards, dones, infos = self.env.step(actions)
            next_global_state = infos['global_state']
    ...
    def evaluate(self, num_episodes: int = 10) -> float:
        ...
        for episode in range(num_episodes):
            ...
            while True:
                ...
                # 贪婪策略（无探索）
                actions = self.algorithm.select_actions(obs_tensor, epsilon=0.0, avail_actions=avail_actions)
    ...
    def save_checkpoint(self, episode: int):
        """保存检查点"""
        checkpoint_path = f"checkpoints/qmix_episode_{episode}.pt"
