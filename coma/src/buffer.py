...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            # 如果直接转换失败，手动处理
            logger.warning(f"观测数据转换失败，尝试手动处理: {e}")
            batch_obs_tensor = torch.zeros(len(batch_obs), self.n_agents, self.obs_dim, device=self.device)
            batch_next_obs_tensor = torch.zeros(len(batch_next_obs), self.n_agents, self.obs_dim, device=self.device)

            for i, (obs, next_obs) in enumerate(zip(batch_obs, batch_next_obs)):
                if isinstance(obs, np.ndarray):
                    if obs.ndim == 2:  # (n_agents, obs_dim)
                        batch_obs_tensor[i] = torch.FloatTensor(obs)
                    elif obs.ndim == 1:  # (obs_dim,)
                        batch_obs_tensor[i, 0] = torch.FloatTensor(obs)

                if isinstance(next_obs, np.ndarray):
                    if next_obs.ndim == 2:  # (n_agents, obs_dim)
                        batch_next_obs_tensor[i] = torch.FloatTensor(next_obs)
                    elif next_obs.ndim == 1:  # (obs_dim,)
                        batch_next_obs_tensor[i, 0] = torch.FloatTensor(next_obs)
