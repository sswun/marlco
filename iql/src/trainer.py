...
class IQLTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 创建缓冲区（IQL不需要全局状态，但为了兼容性保留）
        self.buffer = ReplayBuffer(
            capacity=config['training']['buffer_size'],
            n_agents=self.env_info['n_agents'],
            obs_dim=self.env_info['obs_dims'][0],
            state_dim=1,  # IQL不使用全局状态，设为1
            device=self.device
        )

        # 创建网络
        self.networks = IQLNetworks(self.env_info, config, self.device)

        # 创建算法
        self.algorithm = IQL(self.networks, config, self.device)
    ...
    def collect_episode(self) -> Tuple[float, int]:
        ...
        while True:
            ...
            # 存储经验（IQL不需要全局状态，传入None）
            self.buffer.push(
                obs, actions, rewards, next_obs, dones,
                None, None  # global_state, next_global_state (IQL不需要)
            )
    ...
    def save_checkpoint(self, episode: int):
        """保存检查点"""
        checkpoint_path = f"checkpoints/iql_episode_{episode}.pt"
