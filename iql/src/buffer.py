...
class ReplayBuffer:
    ...
    def push(self, obs: Dict[str, np.ndarray],
        ...
        if global_state is not None:
            ...
            # 如果没有全局状态，创建一个空的占位符
            if self.actual_state_dim is None:
                self.actual_state_dim = 1
                self.max_state_dim = 1
            global_state = np.zeros(self.actual_state_dim)

        if next_global_state is not None:
            next_global_state = np.asarray(next_global_state).flatten()
        else:
            next_global_state = np.zeros_like(global_state)
